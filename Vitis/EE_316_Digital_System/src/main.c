...
//			LCD_write_line("Nathan 1");
			...
			if (adc_channel == a0){
				...
				high = (int)(((float)(current_chanel)/(float)65535)*(float)20000000);
				XTmrCtr_PwmConfigure(&PWM_Timer_inst, 20000000, high);
				xil_printf("high variable: %d\r\n", high);
			}else if (adc_channel == a1){
...
//			LCD_write_line("Nathan 2 ABC");
    		...
    		XTmrCtr_PwmDisable(&PWM_Timer_inst);
    		Xil_Out32(PWM_ip_base_address, 51);
    		delayms(500);
    	}
    }
}
